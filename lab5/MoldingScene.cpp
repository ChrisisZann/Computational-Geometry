...
bool MoldingScene::isFreeToMove(C2DVector &dv)
{
    ...
    for (int i = 0; i < (int) m_pts.size()-1; ++i) 
    {
		...
		Circle2D *cd = new Circle2D ( side.GetMidPoint().x, side.GetMidPoint().y, 20, vvr::Colour::green);
		
		double start = math::Atan( (p2.y-p1.y)/(p2.x-p1.x) );
		double end   = start + math::pi;

		if (end < start) cd->setRange(end, start);
		else			 cd->setRange(start, end);

		m_canvas.add(cd);

		if (i==0){ max = start; min = end; }
		else
		{
	...
	if ( min > max ) free_to_move = false;
	else
	{
		dv.i = math::Sin((max+min)/2 );
		dv.j = math::Cos((max+min)/2 );
		return true;
	}
}
